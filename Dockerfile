...
# its dependency tree, so `/runtime`'s and `/contracts`' node_modules must exist independently —
# both for `tsc` to resolve the sibling source it typechecks (jose, @opentelemetry/api) and for
...
# into them, so without the targets the links dangle and the first `import '@cloudsforge/db'` or
# `import '@cloudsforge/contracts-events'` fails at run time.
...
# run time. There is no ENV line here on purpose — least of all OUTBOX_SIGNING_SECRET, which is
# what makes the audit mirror unforgeable, and ADMIN_API_SERVICE_TOKEN, which is what lets this
# service ask the ledger to reverse an entry. Either in a leaked image layer is the whole estate.
...
# below SCHEMA_VERSION the `audit_events_chain_uniq` constraint may not exist, so two concurrent
# appenders could fork the estate's audit of record without either failing, and neither may
# `approvals_no_self_approval`, so one operator could approve their own ledger reversal.
