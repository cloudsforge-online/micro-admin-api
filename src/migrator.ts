 ...
 *
 * For this service point 3 is the sharp one. `audit_events_chain_uniq` is what makes the audit log
 * a chain rather than a tree, and `approvals_no_self_approval` is what stands between a single
 * compromised operator credential and a manual ledger reversal. A service that could create them
 * at boot is a service that could start without them.
 */
